...
export default function ProductsPage() {
	...
	const [selectedPriceRanges, setSelectedPriceRanges]: any = useState([]);
	const [sortOption, setSortOption]: any = useState("featured");
	const { addToCart }: any = useCart();
	const { toast } = useToast();

	const handleCategoryChange: any = (category: any) => {
	...
	const handlePriceRangeChange = (range: any) => {
		setSelectedPriceRanges((prev: any) => {
			if (prev.includes(range)) {
				return prev.filter((r: any) => r !== range);
			} else {
				return [...prev, range];
			}
		});
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
				<div>
					...
					<p className="text-muted-foreground">
						Browse our collection of church merchandise.
					</p>
				</div>
									...
									<div className="grid gap-2">
										<div className="flex items-center space-x-2">
											<Checkbox
												...
												onCheckedChange={() =>
													handlePriceRangeChange("under10")
												}
											/>
